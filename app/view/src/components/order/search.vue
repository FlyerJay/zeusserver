      ...
      <el-table :data="stockInfo.row" :row-class-name="tableRowClassName" stripe style="width: 100%" v-loading.body="loading" element-loading-text="拼命加载中" border>
        ...
        <el-table-column prop="value" sortable label="出厂价(元/吨)" width="100px" v-if="checkedTBhead.indexOf('出厂价') > -1">
          <template scope="scope">
            ...
            <i class="iconfont icon-down" v-if="scope.row.adjustValue < 0" style="color:#13CE66;font-size:18px">
              <span style="font-size:8px;position:absolute;right:15px;bottom:10px">{{Math.abs(scope.row.adjustValue)}}</span>
            </i>
            <i class="iconfont icon-up" v-if="scope.row.adjustValue > 0" style="color:#FF4949;font-size:18px">
              <span style="font-size:8px;position:absolute;right:15px;bottom:10px">{{Math.abs(scope.row.adjustValue)}}</span>
            </i>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="purePrice" label="供应商开单价" sortable width="100px" v-if="checkedTBhead.indexOf('供应商开单价') > -1">
          <template scope="scope">
            ...
            <i class="iconfont icon-down" v-if="scope.row.benifitAdjust < 0 && scope.row.purePrice" style="color:#13CE66;font-size:18px">
              <span style="font-size:8px;position:absolute;right:15px;bottom:10px">{{Math.abs(scope.row.benifitAdjust)}}</span>
            </i>
            <i class="iconfont icon-up" v-if="scope.row.benifitAdjust > 0 && scope.row.purePrice" style="color:#FF4949;font-size:18px">
              <span style="font-size:8px;position:absolute;right:15px;bottom:10px">{{Math.abs(scope.row.benifitAdjust)}}</span>
            </i>
          </template>
        </el-table-column>
